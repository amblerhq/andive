...
const StatusWrapper = styled(({color, backgroundColor, ...props}) => <div {...props} />)`
  display: inline-flex;
...
export function Status({className, color, backgroundColor, primary, secondary, secondaryItalic = false}: StatusProps) {
  return (
    <StatusWrapper className={className} color={color} backgroundColor={backgroundColor}>
      {primary && <Body3 color={color}>{primary}</Body3>}
      {secondary && (
        <SecondaryStatus color={color} secondaryItalic={secondaryItalic}>
          {secondary}
        </SecondaryStatus>
      )}
    </StatusWrapper>
